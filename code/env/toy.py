...
class Toy(gym.Env):
    def __init__(self, id=None, seed=None):
        ...
        self.hazard_position = np.array([0.,0.])
    ...
    def reset(self,state=None) -> np.ndarray:
        self.state = np.random.uniform(low=[-3.0, -3.0, 0.5, np.pi / 4], high=[3.0, 3.0, 1.5, 3*np.pi / 4])
        if self.id is not None:
            if state is None:
                self.state = np.array([-1.8, 0.0, 2.0, np.pi/4], dtype=np.float32) 
            else:
                self.state=state
    ...
    def get_info(self, state):
        ...
        hazard_vec = self.hazard_position[:2] - state[:2]
    ...
    def get_constraint_values(self, states):
        ...
        hazard_vec = self.hazard_position[:2] - states[:, :2]
    ...
    def _get_obs(self):
        ...
        i = 0
        x, y = (self.hazard_position[:2] - self.state[:2]) @ rot_mat
        hazard_vec = x + 1j * y
    ...
    def _get_avoidable(self, state):
        ...
        hazard_position = self.hazard_position
    ...
    def plot_map(self, ax, v: float = 2.0, theta: float = np.pi / 4):
        ...
        circle = Circle((self.hazard_position[0], self.hazard_position[1]), self.hazard_size, fill=False, color='k',linewidth=1.5)
        ax.add_patch(circle)
    ...
    def plot_task(self, ax):
        ...
        circle = Circle((self.hazard_position[0], self.hazard_position[1]), self.hazard_size, fill=True, alpha=0.5, color=(0.30,0.52,0.74))
        ax.add_patch(circle)
